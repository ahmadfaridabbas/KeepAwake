...
class KeepAwakeManager: ObservableObject {
    ...
    func stopAwake() {
        ...
        // Terminate caffeinate process safely
    ...
    var timerProgress: Double {
        guard isTimerMode, originalDuration > 0 else { return 0 }
        return timeRemaining / originalDuration
    }
    ...
    deinit {
        print("🧹 KeepAwakeManager deinit - cleaning up...")
        
        // Stop timer first
        timer?.invalidate()
        timer = nil
        
        // Force terminate any running process
        if let process = caffeinateProcess, process.isRunning {
            process.terminate()
            caffeinateProcess = nil
        }
        
        print("✅ KeepAwakeManager cleanup completed")
    }
}
